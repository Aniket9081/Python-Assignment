...
class FunctionMapper:
    ...
    def map_test_data(self, test_df, engine, threshold_factor=np.sqrt(2)):
      ...
      for i, row in test_df.iterrows():
          ...
          deviations = []
          try:
            # Ensure there's a corresponding x in ideal functions
            ideal_row = self.ideal_funcs_df[self.ideal_funcs_df['x'] == x_val]
            if ideal_row.empty:
                raise MappingError(f"No matching x value in ideal functions for x={x_val}")

            for col, func in self.best_fits.items():
                y_pred = ideal_row[func].values[0]
            ...
            if deviations:
                ...
                if min_deviation <= threshold_factor * np.std([d[1] for d in deviations]):
                    results.append({
                        ...
                        'ideal_func': int(best_fit[-1])
                    })
          except MappingError as e:
            print(e)
          except IndexError:
            print(f"Error processing x={x_val}, skipping this row.")
...
class Visualization:
    ...
    def plot_results(self, filename="results.png"):
        ...
        for col in self.train_df.columns[1:]:
            sns.lineplot(x=self.train_df['x'], y=self.train_df[col])
            # , label=f"Train {col}", linewidth=2)

        # Plot ideal functions
        for func in self.ideal_funcs_df.columns[1:]:
            sns.lineplot(x=self.ideal_funcs_df['x'], y=self.ideal_funcs_df[func])
                        #  , label=f"Ideal {func}", linestyle='--', linewidth=2)
...
if __name__ == "__main__":
    ...
    # Load data
    train_loader = TrainDataLoader('train.csv')
    train_loader.load_data()
    train_loader.save_to_db('training_data', engine)

    ideal_loader = IdealFunctionsLoader('ideal.csv')
    ideal_loader.load_data()
    ideal_loader.save_to_db('ideal_functions', engine)

    test_loader = TestDataLoader('test.csv')
...
class TestFunctionMapping(unittest.TestCase):
    ...
    def test_map_test_data(self):
      # Create test data with valid x values present in ideal_funcs_df
      test_df = pd.DataFrame({'x': [1, 2, 3], 'y': [2.1, 4.1, 6.1]})
      self.mapper.find_best_fit()  # Ensure best fits are calculated
      test_results_df = self.mapper.map_test_data(test_df, engine)

      # Assert that mapping produced results
      self.assertGreater(len(test_results_df), 0, "No test data points were successfully mapped.")
